...
export default function ReportPrint() {
  ...
  // Article + title per obligation_id -- same identifier convention as
  // Normative Pipeline and Norm Diagnostic, never the bare id alone.
  ...
  return <div className='max-w-5xl mx-auto bg-white p-8 print:p-4'>
    ...
    <section className='print-section card border p-6 mt-6'><h2 className='font-semibold'>IHR 2005 mapping summary</h2><table className='w-full text-sm'><thead><tr><th>IHR Obligation</th><th>Norm</th><th>L</th><th>Gap</th></tr></thead><tbody>{mapping.slice(0,30).map((m:any,i:number)=><tr key={i}><td>{oblMap[m.obligation_id]?.article || m.obligation_id}{oblMap[m.obligation_id]?.article_title ? ` · ${oblMap[m.obligation_id].article_title}` : ''}</td><td>{m.domestic_norm}</td><td>{m.anchoring_level}</td><td>{m.gap_type}</td></tr>)}</tbody></table></section>
